document.addEventListener('DOMContentLoaded', () => {
   ...
   fetch('php/get_carros.php')
        .then(response => response.json())
        .then(data => {
            const carSelect = document.getElementById('carros');
            data.forEach(car => {
                const option = document.createElement('option');
                option.value = car.id_vehiculo;
                option.textContent = car.placa_vehiculo;
                vehiculoSelect.appendChild(option);
            });
        });

    // Manejar opciones de mecanico

    fetch('php/mecanico1.php')
        .then(response => response.json())
        .then(data => {
            const mecanico1Select = document.getElementById('mecanico1');
            data.forEach(mecanico1 => {
                const option = document.createElement('option');
                option.value = mecanico1.id_mecanico;
                option.textContent = mecanico1.nombre_mecanico;
                mecanico1Select.appendChild(option);
            });
        });
